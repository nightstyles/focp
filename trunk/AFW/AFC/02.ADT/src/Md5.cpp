...
/* F, G, H and I are basic CacheMd5 functions.
 */
...
/* ROTATE_LEFT rotates x left n nBits.
 */
...
{
	CMd5Context oContext;
	InitMd5(&oContext);
	UpdateMd5(&oContext, sInput, nInLen);
	EndMd5(&oContext, sOutput);
}
...
{
	uint32 i = 0, nIdx, nLen;
	/* Compute number of bytes mod 64 */
	nIdx = (uint32)((pContext->count[0] >> 3) & 0x3F);

	/* Update number of nBits */
	if ((pContext->count[0] += ((uint32)nInLen << 3))
			< ((uint32)nInLen << 3))
		pContext->count[1]++;
	pContext->count[1] += ((uint32)nInLen >> 29);
	...
	if (nInLen >= nLen)
	{
		CBinary::MemoryCopy((uint8*)&pContext->buffer[nIdx], (uint8*)sInput, nLen);
		Md5Transform(pContext->state, pContext->buffer);
		for (i = nLen; i + 63 < nInLen; i += 64)
			Md5Transform(pContext->state, sInput+i);
		nIdx = 0;
	}
	/* Buffer remaining sInput */
	CBinary::MemoryCopy((uint8*)&pContext->buffer[nIdx], (uint8*)&sInput[i], nInLen-i);
}
...
ADT_API void EndMd5(CMd5Context* pContext, uint8 sOutput[16])
{
	...
	/* Save number of nBits */
	Encode (nBits, pContext->count, 8);

	/* Pad out to 56 mod 64.*/
	nIdx = (uint32)((pContext->count[0] >> 3) & 0x3f);
	nPadLen = (nIdx < 56) ? (56 - nIdx) : (120 - nIdx);
	UpdateMd5 (pContext, PADDING, nPadLen);

	/* Append length (before padding) */
	UpdateMd5 (pContext, nBits, 8);

	/* Store state in sOutput */
	Encode (sOutput, pContext->state, 16);

	/* Zeroize sensitive information.*/
	CBinary::MemorySet((uint8*)pContext, 0, sizeof (*pContext));
}
...
static void Md5Transform(uint32 state[4], const uint8 block[64])
{
	...
	/* Zeroize sensitive information.*/
	CBinary::MemorySet((uint8*)x, 0, sizeof (x));
}
