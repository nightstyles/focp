...
static const jc_uint CRCTable[256] = 
{
...
{
	jc_uint i, nCrc32 = 0xFFFFFFFF;
	jc_uchar* pBuf = (jc_uchar*)pKey;
	for(i=0; i<nKeyLen; i++)
...
static const jc_uint nPrimeTableLength = sizeof(pPrimes)/sizeof(pPrimes[0]);
static const jc_float nMaxLoadFactor = (const jc_float)0.65;

/*****************************************************************************/
/*GetHashIndex*/
/*****************************************************************************/
static jc_uint GetHashIndex(jc_uint nTableLength, jc_uint nHashValue)
{
...
{
	jc_uint nSize = pPrimes[0];
	CJcHashTable *pHashTable = New(CJcHashTable);
	pHashTable->pBuckets = New2(CJcHashEntry*, nSize);
	pHashTable->nTableLength = nSize;
	pHashTable->nPrimeIndex = 0;
	pHashTable->BucketSize = 0;
	pHashTable->nLoadLimit = (jc_uint) g_oInterface.Ceil(nSize * nMaxLoadFactor);
...
{
	jc_uint i;
	CJcHashEntry **pBuckets = pHashTable->pBuckets;
	for (i = 0; i < pHashTable->nTableLength; i++)
	{
		CJcHashEntry * pBucket = pBuckets[i];
		while (NULL != pBucket)
		{
			CJcHashEntry* pNext = pBucket->pNext;
			if(pBucket->pKey)
...
jc_uint GetHashValue(CJcHashTable *pHashTable, jc_void *pKey)
{
	...
	* - logic taken from java 1.4 hashtable source */
	jc_uint nHashValue = pHashTable->HashFunc(pKey);
...
/*ExpandHashTable*/
/*****************************************************************************/
static jc_void ExpandHashTable(CJcHashTable *pHashTable)
{
	/* Double the size of the Bucket to accomodate more entries */
	jc_uint nNewSize, i;
	CJcHashEntry **pNewBuckets;
	/* Check we're not hitting max capacity */
	if (pHashTable->nPrimeIndex == (nPrimeTableLength - 1))
		return;
	nNewSize = pPrimes[++(pHashTable->nPrimeIndex)];	
	pNewBuckets = (CJcHashEntry **)g_oInterface.Realloc(pHashTable->pBuckets, nNewSize * sizeof(CJcHashEntry *));
	pHashTable->pBuckets = pNewBuckets;
	MemorySet(pNewBuckets+pHashTable->nTableLength, 0, (nNewSize - pHashTable->nTableLength)*sizeof(CJcHashEntry*));
	for (i = 0; i < pHashTable->nTableLength; i++)
	{
		CJcHashEntry **pBuckets = pNewBuckets + i;
		CJcHashEntry *pBucket = *pBuckets;
		for (; pBucket != NULL; pBucket = *pBuckets)
		{
			jc_uint nIndex = GetHashIndex(nNewSize,pBucket->nHashValue);
			if (nIndex == i)
	...
	pHashTable->nLoadLimit = (jc_uint) g_oInterface.Ceil(nNewSize * nMaxLoadFactor);
}
