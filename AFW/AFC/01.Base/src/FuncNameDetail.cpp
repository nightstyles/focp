...
BOOL StackWalker::GetFuncionName(HANDLE hThread, const CONTEXT *context, PReadProcessMemoryRoutine readMemoryFunction, LPVOID pUserData)
{
	CONTEXT c;;
	CallstackEntry csEntry;
	IMAGEHLP_SYMBOL64 *pSym = NULL;
...
#error "Platform not supported!"
	...
	this->OnCallstackEntry2(et, csEntry);
...
void StackWalker::OnCallstackEntry2(CallstackEntryType eType, CallstackEntry &entry)
{
	CHAR buffer[STACKWALK_MAX_NAMELEN];
	if ( (eType != lastEntry) && (entry.offset != 0) )
	{
		...
		if (entry.undFullName[0] != 0)
			strcpy_s(entry.name, entry.undFullName);
		_snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "%s", entry.name);
		OnOutput(buffer);
	}
}
