...
{
	uint32 nCrc=*pCrc32, i;

	if(bCaseSentive)
	{
		for(i=0; i<nCount; i++)
			nCrc = (nCrc>>8) ^ CRCTable[(pBuf[i] ^ nCrc) & 0xFF];
	}
	else for(i=0; i<nCount; i++)
	{
		...
		if(ch >= 'a' && ch <= 'z')
			ch = ch - 'a' + 'A';
		nCrc = (nCrc>>8) ^ CRCTable[(ch ^ nCrc) & 0xFF];
	}
	*pCrc32 = nCrc;
}
...
ADT_API uint32 GetCrc32(const uint8 *pBuf, uint32 nCount, uint32 bCaseSentive)
{
	uint32 nCrc;
	InitCrc32(&nCrc);
	ComputeCrc32(&nCrc, pBuf, nCount, bCaseSentive);
	EndCrc32(&nCrc);
	return nCrc;
}
