...
static const CDesBlock weak_keys[NUM_WEAK_KEY]=
{
	/* weak keys */
...
static const uint32 des_skb[8][64]=
{
...
static const uint32 DES_SPtrans[8][64]=
{
	{/* nibble 0 */
...
/*static void DES_set_odd_parity(CDesBlock *key)
{
	uint32 i;

	for (i=0; i<sizeof(CDesBlock); i++)
		(*key)[i]=odd_parity[(*key)[i]];
}*/

static int DES_check_key_parity(const uchar key[8])
{
	uint32 i;
...
static int DES_is_weak_key(const uchar key[8])
{
	int i;
	for (i=0; i<NUM_WEAK_KEY; i++)
		if (CBinary::MemoryCompare(weak_keys[i],key,8) == 0)
			return(1);
	return(0);
}

static void DES_encrypt1(uint32 *data, CDesKey *ks, bool enc)
{
	register uint32 l,r,t,u;
	register uint32 *s;
	...
	if (enc)
	{
		D_ENCRYPT(l,r, 0, u, t); /*  1 */
		D_ENCRYPT(r,l, 2, u, t); /*  2 */
		D_ENCRYPT(l,r, 4, u, t); /*  3 */
		D_ENCRYPT(r,l, 6, u, t); /*  4 */
		D_ENCRYPT(l,r, 8, u, t); /*  5 */
		D_ENCRYPT(r,l,10, u, t); /*  6 */
		D_ENCRYPT(l,r,12, u, t); /*  7 */
		D_ENCRYPT(r,l,14, u, t); /*  8 */
		D_ENCRYPT(l,r,16, u, t); /*  9 */
		D_ENCRYPT(r,l,18, u, t); /*  10 */
		D_ENCRYPT(l,r,20, u, t); /*  11 */
		D_ENCRYPT(r,l,22, u, t); /*  12 */
		D_ENCRYPT(l,r,24, u, t); /*  13 */
		D_ENCRYPT(r,l,26, u, t); /*  14 */
		D_ENCRYPT(l,r,28, u, t); /*  15 */
		D_ENCRYPT(r,l,30, u, t); /*  16 */
	}
	else
	{
		D_ENCRYPT(l,r,30, u, t); /* 16 */
		D_ENCRYPT(r,l,28, u, t); /* 15 */
		D_ENCRYPT(l,r,26, u, t); /* 14 */
		D_ENCRYPT(r,l,24, u, t); /* 13 */
		D_ENCRYPT(l,r,22, u, t); /* 12 */
		D_ENCRYPT(r,l,20, u, t); /* 11 */
		D_ENCRYPT(l,r,18, u, t); /* 10 */
		D_ENCRYPT(r,l,16, u, t); /*  9 */
		D_ENCRYPT(l,r,14, u, t); /*  8 */
		D_ENCRYPT(r,l,12, u, t); /*  7 */
		D_ENCRYPT(l,r,10, u, t); /*  6 */
		D_ENCRYPT(r,l, 8, u, t); /*  5 */
		D_ENCRYPT(l,r, 6, u, t); /*  4 */
		D_ENCRYPT(r,l, 4, u, t); /*  3 */
		D_ENCRYPT(l,r, 2, u, t); /*  2 */
		D_ENCRYPT(r,l, 0, u, t); /*  1 */
	}
...
void DES_encrypt2(uint32 *data, CDesKey *ks, bool enc)
{
	register uint32 l,r,t,u;
	register uint32 *s;
...
static void DES_encrypt3(uint32 *data, CDesKey *ks1, CDesKey *ks2, CDesKey *ks3, bool enc)
{
	register uint32 l,r;
	...
	data[0]=l;
	data[1]=r;
	DES_encrypt2(data,ks1,enc);
	DES_encrypt2(data,ks2,!enc);
	DES_encrypt2(data,ks3,enc);
	l=data[0];
	r=data[1];
	FP(r,l);
	data[0]=l;
...
ADT_API int32 SetDesKey(CDesKey* schedule, const uchar key[8], bool bCheck)
{
	static const int32 shifts2[16]= {0,0,1,1,1,1,1,1,0,1,1,1,1,1,1,0};
	register uint32 c,d,t,s,t2;
	register const uchar *in;
	register uint32 *k;
	register int32 i;
	if(bCheck)
	{
	...
	PERM_OP (d,c,t,1,0x55555555);
	d=	(((d&0x000000ffL)<<16)| (d&0x0000ff00) | ((d&0x00ff0000L)>>16)|((c&0xf0000000)>>4));
	c&=0x0fffffffL;
	...
	for (i=0; i<16; i++)
	{
		...
		s=	des_skb[0][ (c    )&0x3f                ]|
			...
			des_skb[3][((c>>20)&0x01)|((c>>21)&0x06) |
					   ((c>>22)&0x38)];
		t=	des_skb[4][ (d    )&0x3f                ]|
			des_skb[5][((d>> 7)&0x03)|((d>> 8L)&0x3c)]|
			des_skb[6][ (d>>15)&0x3f                ]|
			des_skb[7][((d>>21)&0x0f)|((d>>22L)&0x30)];
...
ADT_API void DesCbc(const uchar *in, uchar *out, int32 length, CDesKey *_schedule, uchar ivec[8], bool enc)
{
	register uint32 tin0,tin1;
	register uint32 tout0,tout1,xor0,xor1;
	register int32 l=length;
	uint32 tin[2];
	uchar *iv;
	...
	if (enc)
	{
		...
		for (l-=8; l>=0; l-=8)
		{
			...
			tin1^=tout1;
			tin[1]=tin1;
			DES_encrypt1((uint32 *)tin,_schedule,enc);
			tout0=tin[0];
			l2c(tout0,out);
			tout1=tin[1];
			l2c(tout1,out);
...
ADT_API void DesEde3Cbc(const uchar *input, uchar *output, int32 length, CDesKey pKey[3], uchar ivec[8], bool enc)
{
	register uint32 tin0,tin1;
	register uint32 tout0,tout1,xor0,xor1;
	register const uchar *in;
	...
	CDesKey *ks1 = pKey, *ks2=ks1+1, *ks3=ks2+1;
	...
	if (enc)
	{
		...
		for (l-=8; l>=0; l-=8)
		{
			...
			tin[0]=tin0;
			tin[1]=tin1;
			DES_encrypt3((uint32 *)tin,ks1,ks2,ks3, enc);
			tout0=tin[0];
			tout1=tin[1];
